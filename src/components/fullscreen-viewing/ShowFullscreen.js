...
let ShowFullscreeen = (props) => {
	...
	let regexp = /[:T-]/g;
	let created = currentPhoto.created_at.split(regexp).splice(0, 3);

	function Like() {
		let [likes, setLikes] = useState(currentPhoto.likes);

		for (let i = 0; i < usersLikes.length; i++) {
			if (currentPhoto.id === usersLikes[i].id) {
				return(
					<div className="fullscreen-viewing__likes likes"
						onClick={() => {
